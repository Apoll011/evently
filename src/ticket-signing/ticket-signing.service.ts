...
import { createHash, sign, verify, createPrivateKey, createPublicKey, KeyObject } from 'node:crypto';
...
export class TicketSigningService implements OnModuleInit {
	...
	async onModuleInit() {
		...
		try {
			...
			throw new Error(`TicketSigningService: failed to read signing keys from ./keys — ${(err as Error).message}`);
		}
	...
	verifyHash(ticket: TicketHash, signature: string): boolean {
		try {
			...
			return verify(null, packed, this.publicKey, Buffer.from(signature, 'base64url'));
		} catch {
			return false;
		}
	}
	...
	private hashField(value: string | null): string {
		...
		const content = value === null ? Buffer.alloc(0) : Buffer.from(value, 'utf8');
	...
	private hashCustomFields(fields: FieldValue[]): string {
		...
		return createHash('sha256').update(canonical, 'utf8').digest('hex').slice(0, HASH_BYTES * 2);
	}
	...
	private idToBuffer(id: string): Buffer {
		...
		if (bytes.length >= UUID_MARKER) {
			throw new Error(`Identifier too long to encode (${bytes.length} bytes): ${id}`);
		}
		return Buffer.concat([Buffer.from([bytes.length]), bytes]);
	}

	private bufferToId(buf: Buffer, offset: number): { id: string; next: number } {
		if (offset >= buf.length) throw new Error('Ticket payload truncated (id marker)');
		const marker = buf.readUInt8(offset);

		if (marker === UUID_MARKER) {
			if (offset + 17 > buf.length) throw new Error('Ticket payload truncated (uuid)');
		...
		const len = marker;
		if (offset + 1 + len > buf.length) throw new Error('Ticket payload truncated (string id)');
	...
	private hashHexToBuffer(hex: string): Buffer {
		...
		if (buf.length !== HASH_BYTES) {
			throw new Error(`Invalid hash length: expected ${HASH_BYTES} bytes, got ${buf.length}`);
		}
		return buf;
	}

	private readHash(buf: Buffer, offset: number): string {
		if (offset + HASH_BYTES > buf.length) throw new Error('Ticket payload truncated (hash)');
